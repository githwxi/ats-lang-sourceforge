...
let position_of (p: Lexing.position): position =
...
let min_position_position (p1: position) (p2: position) =
  if p1.char_number <= p2.char_number then p1 else p2

let max_position_position (p1: position) (p2: position) =
  if p1.char_number >= p2.char_number then p1 else p2

let fprint_position (out: out_channel) (p: position): unit =
  P.fprintf out "%d (line=%d, offset=%d)" p.char_number p.line_number p.line_offset

let sprintf_position (p: position): string =
  P.sprintf "%d (line=%d, offset=%d)" p.char_number p.line_number p.line_offset
...
let to_string (l: location): string =
  error_of_unavailability "ats_location: to_string"
...
let get_the_comment_start_position (): position = !the_comment_start_position
let set_the_comment_start_position (lb: Lexing.lexbuf): unit =
  the_comment_start_position := position_of (Lexing.lexeme_start_p lb)
let reset_the_comment_start_position (): unit =
  the_comment_start_position := nonpos

(* ****** ****** *)

let get_the_comment_rest_start_position (): position option =
  !the_comment_rest_start_position
let set_the_comment_rest_start_position (lb: Lexing.lexbuf): unit =
  the_comment_rest_start_position := Some (position_of (Lexing.lexeme_start_p lb))
let reset_the_comment_rest_start_position (): unit =
