...
module NameSpace: NameSpaceType = struct
  ...
  let add (n: name) = (the_names := n :: !the_names)
  let add_list (ns: name list) = (the_names := List.append ns !the_names)

  let get_names (): name list = !the_names
  let get_all_names (): name list =
  ...
  let push (): unit = begin
    the_names_list := !the_names :: !the_names_list;
    the_names := [];
  end
