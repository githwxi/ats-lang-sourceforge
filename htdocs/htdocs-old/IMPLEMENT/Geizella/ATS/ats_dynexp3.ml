...
let rec fprint_pat3 (out: out_channel) (p3t0: pat3): unit =
  match p3t0.pat3_node with
    | PT3any d2v -> P.fprintf out "PT3any(%a)" fprint_dvar2 d2v
    | PT3bool b -> P.fprintf out "PT3bool(%b)" b
    | PT3char c -> P.fprintf out "PT3char(%c)" c
    | PT3con (freeknd, d2c, (npf, p3ts)) -> P.fprintf out
	"PT3con(%a; %i; %a)" fprint_dcon2 d2c npf fprint_pat3_list p3ts
    | PT3empty -> P.fprintf out "PT3empty"
    | PT3int i -> P.fprintf out "PT3int(%a)" fprint_intinf i
    | PT3lst p3ts -> P.fprintf out "PT3lst(%a)" fprint_pat3_list p3ts
    | PT3rec (is_flat, (npf, lp3ts)) ->
	P.fprintf out "PT3rec{%i; %a}" npf fprint_labpat3_list lp3ts
    | PT3string s -> P.fprintf out "PT3string(%s)" s
    | PT3var (isref, d2v, op3t) -> begin match op3t with
        | Some p3t -> begin
            P.fprintf out "PT3var(";
    ...
    | PT3vbox d2v -> P.fprintf out "PT3vbox(%a)" fprint_dvar2 d2v 
...
and fprint_labpat3 (out: out_channel) ((l, p3t): labpat3): unit =
  P.fprintf out "%a= %a" Lab.fprint l fprint_pat3 p3t
...
let rec fprint_dexp3 (out: out_channel) (d3e0: dexp3): unit =
  match d3e0.dexp3_node with
	...
	P.fprintf out "DE3app_dyn(%a; %i; %a)"
	  fprint_dexp3 d3e_fun npf fprint_dexp3_list d3es_arg
    | DE3app_sta d3e -> P.fprintf out "DE3app_sta(%a)" fprint_dexp3 d3e
    | DE3arr _ -> P.fprintf out "DE3arr(...)"
    | DE3assgn_ptr (d3e_ptr, d3labs, d3e_val) ->
	P.fprintf out "DE3assgn_ptr(%a; ...)" fprint_dexp3 d3e_ptr
    | DE3assgn_var (d2v_ptr, d3labs, d3e_val) ->
	P.fprintf out "DE3assgn_var(%a; ...)" fprint_dvar2 d2v_ptr
    | DE3case (sgn, d3es, c3ls) -> P.fprintf out "DE3case(%a; %a)"
	fprint_dexp3_list d3es fprint_cla3_list c3ls
    | DE3char c -> P.fprintf out "DE3char(%c)" c
    | DE3con (d2c, (npf, d3es)) -> P.fprintf out "DE3con(%a; %i; %a)"
	fprint_dcon2 d2c npf fprint_dexp3_list d3es
    | DE3cst d2c -> P.fprintf out "DE3cst(%a)" fprint_dcst2 d2c
    | DE3crypt (knd, d3e) -> P.fprintf out "DE3crypt(%i; %a)" knd fprint_dexp3 d3e
    | DE3delay (lin, d3e) -> P.fprintf out "DE3delay(%i; %a)" lin fprint_dexp3 d3e
    | DE3dynload f -> P.fprintf out "DE3dynload(%a)" Fil.fprint f
    | DE3empty -> P.fprintf out "DE3empty"
    | DE3extval code -> P.fprintf out "DE3extval(%s)" code
    | DE3fix (d2v_fun, d3e_body) -> P.fprintf out "DE3fix(%a; %a)"
	fprint_dvar2 d2v_fun fprint_dexp3 d3e_body
    | DE3float s -> P.fprintf out "DE3float(%s)" s
    | DE3fold d3e -> P.fprintf out "DE3fold(%a)" fprint_dexp3 d3e
    | DE3foldat d3e -> P.fprintf out "DE3foldat(%a)" fprint_dexp3 d3e
    | DE3freeat d3e -> P.fprintf out "DE3freeat(%a)" fprint_dexp3 d3e
    | DE3for (d3e_init, d3e_test, d3e_post, d3e_body) ->
	P.fprintf out "DE3for(%a; %a; %a; %a)"
	  fprint_dexp3 d3e_init fprint_dexp3 d3e_test
	  fprint_dexp3 d3e_post fprint_dexp3 d3e_body
    | DE3if (d3e_cond, d3e_then, d3e_else) -> P.fprintf out "DE3if(%a; %a; %a)"
	fprint_dexp3 d3e_cond fprint_dexp3 d3e_then fprint_dexp3 d3e_else
    | DE3int (ik, i) -> P.fprintf out "DE3int(%a)" fprint_intinf i
    | DE3lam_dyn (is_lin, (npf, p3ts_arg), d3e_body) ->
	P.fprintf out "DE3lam_dyn(%b; %i; %a; %a)"
	  is_lin npf fprint_pat3_list p3ts_arg fprint_dexp3 d3e_body
    | DE3lam_sta (s2vs, s2ps, d3e) -> P.fprintf out "DE3lam_sta(%a; %a; %a)"
	fprint_svar2_list s2vs fprint_sexp2_list s2ps fprint_dexp3 d3e
    | DE3lam_sta_para (s2vs, s2ps, d3e) -> P.fprintf out "DE3lam_sta_para(%a; %a; %a)"
	fprint_svar2_list s2vs fprint_sexp2_list s2ps fprint_dexp3 d3e
    | DE3let (d3cs, d3e) -> P.fprintf out "DE3let(%a; %a)"
	fprint_dec3_list d3cs fprint_dexp3 d3e
    | DE3loopexn i ->
	if i > 0 then P.fprintf out "break" else P.fprintf out "continue"
    | DE3lst d3es -> P.fprintf out "DE3lst(%a)" fprint_dexp3_list d3es
    | DE3mod _ -> P.fprintf out "DE3mod(...)"
    | DE3ptrof_ptr (d3e_ptr, d3labs) ->
	P.fprintf out "DE3ptrof_ptr(%a; ...)" fprint_dexp3 d3e_ptr
    | DE3ptrof_var (d2v_ptr, d3labs) ->
	P.fprintf out "DE3ptrof_var(%a; ...)" fprint_dvar2 d2v_ptr
    | DE3raise d3e_exn -> P.fprintf out "DE3raise(%a)" fprint_dexp3 d3e_exn
    | DE3rec (is_flat, (npf, ld3es)) ->
	P.fprintf out "DE3rec{%i; %a}" npf fprint_labdexp3_list ld3es
    | DE3refarg (refval, freeknd, d3e) -> P.fprintf out "DE3refarg(%i; %i; %a)"
	refval freeknd fprint_dexp3 d3e
    | DE3sel (d3e, d3labs) -> P.fprintf out "DE3sel(%a; ...)" fprint_dexp3 d3e
    (* dynamic linear record ptr selection *) 
    | DE3sel_ptr (d3e, d3labs) -> P.fprintf out "DE3sel_ptr(%a; ...)"
	fprint_dexp3 d3e
    | DE3sel_var (d2v, d3labs) ->
	P.fprintf out "DE3sel_var(%a; ...)" fprint_dvar2 d2v
    | DE3seq d3es -> P.fprintf out "DE3seq(%a)" fprint_dexp3_list d3es
    | DE3sif (s2e_cond, d3e_then, d3e_else) -> P.fprintf out "DE3sif(%a; %a; %a;)"
	fprint_sexp2 s2e_cond fprint_dexp3 d3e_then fprint_dexp3 d3e_else	
    | DE3string s -> P.fprintf out "DE3string(%s)" s
    | DE3struct (ld3es) -> P.fprintf out "DE3struct{%a}" fprint_labdexp3_list ld3es
    | DE3template_cst (d2c, s2ess) -> P.fprintf out "DE3template_cst(%a; %a)"
	fprint_dcst2 d2c fprint_sexp2_list_list s2ess
    | DE3template_var (d2v, s2ess) -> P.fprintf out "DE3template_var(%a; %a)"
	fprint_dvar2 d2v fprint_sexp2_list_list s2ess
    | DE3top -> P.fprintf out "DE3top"
    | DE3trywith (d3e, c3ls) -> P.fprintf out "DE3trywith(%a; %a)"
	fprint_dexp3 d3e fprint_cla3_list c3ls
    | DE3unfold d3e -> P.fprintf out "DE3unfold(%a)" fprint_dexp3 d3e
    | DE3var d2v -> P.fprintf out "DE3var(%a)" fprint_dvar2 d2v
    | DE3viewat_assgn_ptr (d3e_ptr, d3labs, d3e_val) ->
	P.fprintf out "DE3viewat_assgn_ptr(%a; ...)" fprint_dexp3 d3e_ptr
    | DE3viewat_assgn_var (d2v_ptr, d3labs, d3e_val) ->
	P.fprintf out "DE3viewat_assgn_var(%a; ...)" fprint_dvar2 d2v_ptr
    | DE3viewat_ptr (d3e, d3labs, d2v_view, s2labs_view) ->
	P.fprintf out "DE3viewat_ptr(%a; ...)" fprint_dexp3 d3e
    | DE3viewat_var (d2v, d3labs, d2v_view, s2labs_view) ->
	P.fprintf out "DE3viewat_var(%a; ...)" fprint_dvar2 d2v
    | DE3where (d3e, d3cs) ->
	P.fprintf out "DE3where(%a; ...)" fprint_dexp3 d3e
    | DE3while (d3e_test, d3e_body) -> P.fprintf out
...
and fprint_labdexp3 (out: out_channel) ((l, d3e): labdexp3): unit =
  P.fprintf out "%a= %a" Lab.fprint l fprint_dexp3 d3e
...
and fprint_dec3 (out: out_channel) (d: dec3): unit =
  match d.dec3_node with
	...
	P.fprintf out "DC3extype(%s, %a)" name fprint_sexp2 s2e_def
    | DC3extval (name, d3e_def) ->
	P.fprintf out "DC3extval(%s, %a)" name fprint_dexp3 d3e_def
    | DC3dyncsts _ -> P.fprintf out "DC3dyncsts(...)" 
    | DC3data _ -> P.fprintf out "DC3data(...)" 
    | DC3exns _ -> P.fprintf out "DC3exns(...)" 
    | DC3impls _ -> P.fprintf out "DC3impls(...)" 
    | DC3funs _ -> P.fprintf out "DC3funs(...)"  
    | DC3vals _ -> P.fprintf out "DC3vals(...)"  
    | DC3valpars _ -> P.fprintf out "DC3valpars(...)"  
    | DC3valrecs _ -> P.fprintf out "DC3valrecs(...)"  
    | DC3vars _ -> P.fprintf out "DC3vars(...)"
    | DC3local _ -> P.fprintf out "DC3local(...)"
    | DC3staload (f, _) -> P.fprintf out "DC3staload(%a)" Fil.fprint f
    | DC3dynload f -> P.fprintf out "DC3dynload(%a)" Fil.fprint f
    | DC3extern _ -> P.fprintf out "DC3extern(...)"
...
let seff2_dec3_var_union (sf2e: seff2) (d: dec3_var): seff2 =
  match d.dec3_var_ini with
    | None -> sf2e
    | Some d3e -> seff2_dexp3_union sf2e d3e

let seff2_dec3_var_list_union (sf2e: seff2) (ds: dec3_var list): seff2 =
...
let dexp3_crypt loc (s2e: sexp2) (knd: int) (d3e: dexp3): dexp3 = {
  dexp3_loc= loc;
...
let dexp3_fix loc (d2v_fun: dvar2) (d3e_def: dexp3): dexp3 = {
  dexp3_loc= loc;
...
let dexp3_float_with_type loc (s2e: sexp2) (f: string): dexp3 = {
  dexp3_loc= loc;
...
let dexp3_let_simplify loc (d3cs: dec3 list) (d3e: dexp3): dexp3 =
  match d3cs with
    | [] -> d3e
    | _ :: _ -> dexp3_let loc d3cs d3e
...
let dexp3_rec_simplify loc
  ...
  end else dexp3_rec loc s2e is_flat npf ld3es
...
let dexp3_viewat_ptr loc (s2e: sexp2) (d3e: dexp3)
  (d3labs: dlab3 list) (d2v_view: dvar2) (s2labs_view: slab2 list)
  : dexp3 = {
    dexp3_loc= loc;
...
let dexp3_where_with_seff loc
  (sf2e: seff2) (d3e: dexp3) (d3cs: dec3 list): dexp3 = {
    dexp3_loc= loc;
    dexp3_node= DE3where (d3e, d3cs);
    dexp3_is_proof= false;
    dexp3_seff= sf2e;
    dexp3_type= d3e.dexp3_type;
  }

let dexp3_where loc (d3e: dexp3) (d3cs: dec3 list): dexp3 =
  let sf2e = seff2_nil in
  let sf2e = seff2_dec3_list_union sf2e d3cs in
  let sf2e = seff2_dexp3_union sf2e d3e in
    dexp3_where_with_seff loc sf2e d3e d3cs

let dexp3_where_simplify loc d3e d3cs: dexp3 =
  match d3cs with
    | [] -> d3e
    | _ :: _ -> dexp3_where loc d3e d3cs
...
let dec3_vals loc (vk: Syn.valkind) (ds: dec3_val list): dec3 = {
...
let dec3_vars loc (is_stack: bool) (ds: dec3_var list): dec3 = {
...
let dec3_staload loc
  (f: Fil.filename) (ods3: (dec3 list) option): dec3 = {
   dec3_loc= loc;
   dec3_node= DC3staload (f, ods3);
 }

let dec3_dynload loc (f: Fil.filename): dec3 = {
   dec3_loc= loc; dec3_node= DC3dynload f;
 }

(* ****** ****** *)

let dec3_extern loc (position: int) (code: string): dec3 = {
...
let dexp3_of_ditem2 loc (d2i: ditem2): dexp3 =
  match d2i with
    ...
    | _ -> begin
	P.fprintf stderr "%a: dexp3_of_ditem2: %a\n"
