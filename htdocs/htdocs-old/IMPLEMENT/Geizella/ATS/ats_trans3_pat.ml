...
let error_with_loc (loc: loc) (s: string): 'a = begin
  prerr_string "ats_pat_trans3: "; Err.error_with_loc loc s;
end
...
and pat2_var_tr_up loc0
  (isref: bool) (d2v: dvar2) (op2t: pat2 option): pat3 =
  begin match op2t with
    | Some p2t ->
	...
	let loc0 = p2t0.pat2_loc in match p2t0.pat2_node with
	  | PT2var (isref, d2v, op2t) -> begin
		...
		if refval = 0 then true else sexp2_is_proof s2e_arg in
		  ...
		  let s2e_addr = sexp2_var s2v_addr in
		  let () = SEnv3.svar_add s2v_addr in
		  let () = d2v.dvar2_is_alias <- true in
...
let patcst2_intc_hypo_add (s2i: sexp2) (xs: intinf list): unit =
  List.iter
    (function x ->
       SEnv3.hypo_prop_add (Ineq.make_exp (Some [s2i; sexp2_intinf x])))
    xs
