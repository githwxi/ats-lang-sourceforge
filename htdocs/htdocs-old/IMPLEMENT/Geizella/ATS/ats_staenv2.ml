...
let error_with_loc (loc: loc) (s: string): 'a =
  Err.error_with_loc loc ("Ats_staenv2: " ^ s)
...
module SEXP2arg: sig type item end with type item = sitem2 = struct
  type item = sitem2
end
...
let sexp2_env_add_fil (id: Syn.sid) (f: Fil.filename): unit =
  let s = Syn.sym_of_sid id in
    SEXP2env.add (Sym.staload_symbol s) (SI2fil f)
