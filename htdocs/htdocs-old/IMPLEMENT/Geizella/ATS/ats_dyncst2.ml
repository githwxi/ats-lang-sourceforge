...
let fun_dynload_dummy_name = Sym.make_with_string "dynload_dummy"
let dcst2_fun_dynload_dummy_is_implemented (): bool =
  match DENV.DEXP2env.find_in_pervasives fun_dynload_dummy_name with
    | Some (DI2cst d2c) -> begin
	match d2c.dcst2_def with Some _ -> true | None -> false
      end
    | _ -> error "dcst2_fun_dynload_dummy"
