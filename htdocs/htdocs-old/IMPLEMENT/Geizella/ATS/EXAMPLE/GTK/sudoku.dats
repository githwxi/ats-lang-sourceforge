...
fn menu_button_press_event_ref
  ...
  val is_fixed: bool = the_puzzle_ori[i,j] > 0
...
val the_puzzles = array @[string(NMAX12)][
// easy
...
fn puzzle_string_get (): string (NMAX12) =
  let
    val n = natrand48 (array_size the_puzzles)
  in
    the_puzzles[n]
  end
...
*)
    ...
    and loop2 (i: natLt NMAX1, j: natLte NMAX1):<cloptr1> void =
      ...
      in
        the_puzzle_ori[i,j] := digit;
        the_puzzle_cur[i,j] := digit;
        loop2 (i,j+1)
      end else begin
        loop1 (i+1)
      end
...
fn button_matrix_update (): void =
  let
    ...
    and loop2 (i: natLt NMAX1, j: natLte NMAX1): void =
      if j < NMAX1 then let
        val is_fixed: bool = the_puzzle_ori[i,j] > 0
        val r_button = the_button_matrix[i,j]
        val lab =
          if is_fixed then begin
            the_ulabels[the_puzzle_cur[i,j]]
          end else begin
            the_labels[the_puzzle_cur[i,j]]
          end
        val (pf_button | p_button) = g_objref_get r_button
        val () = gtk_button_set_label (GtkButtonIsGtkButton | !p_button, lab)
...
fn button_new_game_make (): [l:addr] (gobj GtkButton @ l | ptr l) =
  let
    fn action {c:gcls} (wid: &gobj GtkButton, data: &void): void =
...
fn button_pause_make (): [l:addr] (gobj GtkButton @ l | ptr l) =
  let
    fn action {c:gcls}
...
fn button_matrix_make_with_menu (): [l:addr] (gobj GtkTable @ l | ptr l) =
  let
    ...
    and loop2 (table: &gobj GtkTable, i: natLt NMAX1, j: natLte NMAX1): void =
      if j < NMAX1 then let
        ...
        val () = the_button_matrix[i,j] := r_button
