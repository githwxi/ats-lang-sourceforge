...
d1ec:
  ...
  | IMPLEMENT impldecseq		{ dec_impls $2 }
  /* including file */
...
srtext: /* extended sort (sort and subset sort) */
  | srt                                 { srtext_srt $1 }
  | LBRACE sid COLON srtext BAR sexpseq RBRACE
                                        { srtext_sub $2 $4 $6 }
;
...
commatmpsargseq:
  | /* empty */                         { [] }
  | COMMA sarg commatmpsargseq          { $2 :: $3 }
;

tmpsargseq: /* static variable sequence */
  | sarg commatmpsargseq                { $1 :: $2 }
;

tmpsargseqseq:
  | tmpsargseq                          { [$1] }
  | tmpsargseq GTLT tmpsargseqseq       { $1 :: $3 }
;

implid:
  | doqid				{ ($1, []) }
  | tmpdoqid tmpsargseqseq GT           { ($1, $2) }
;
