...
let rec gcd_nat (n1: int) (n2: int) = (* n1 and n2 are natural numbers *)
  if n1 > 0 then gcd_nat (n2 mod n1) n1 else n2

let gcd_int (i1: int) (i2: int) = (* i1 and i2 are integers *)
  if i1 >= 0 then
...
let fprint_bool (out: out_channel) (b: bool) = P.fprintf out "%b" b

let fprint_char (out: out_channel) (c: char) = output_char out c

let fprint_int (out: out_channel) (i: int) = P.fprintf out "%i" i

let fprint_intinf (out: out_channel) (i: intinf) =
  P.fprintf out "%s" (string_of_intinf i)

let fprint_string (out: out_channel) (s: string) = output_string out s

let prerr_line (s: string): unit = begin
...
let fprint_list_with_sep f out xs sep =
  let rec aux x xs = match xs with
    | [] -> f out x
    | x' :: xs' -> (f out x; fprint_string out sep; aux x' xs') in
    match xs with [] -> () | x :: xs -> aux x xs

let fprint_intlist (out: out_channel) (ns: int list) =
...
let rec list_find_first (p: 'a -> bool) (xs: 'a list): 'a option =
  match xs with
    | [] -> None
