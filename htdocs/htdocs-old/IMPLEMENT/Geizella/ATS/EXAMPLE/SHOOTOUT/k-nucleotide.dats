...
extern fun dna_count {n,k:nat | k <= n}
  (tbl: symtbl_t, n: int n, k: int k) : symtbl_t
  = "dna_count"
...
// linear append
fun frqlst_append
  (xs0: &frqlst >> frqlst, ys: frqlst): void = begin
  case xs0 of
...
and partition {l00,l01,l1:addr}
  (pf00: symbol_t @ l00, pf01: float @ l01, pf1: frqlst? @ l1 |
   node: FRQLSTcons (l00, l01, l1), node1: ptr l1,
...
// print and free
fun print_frqlst (dna: dna_t, kfs: frqlst): void = begin
  case+ kfs of
...
fun write_frequencies {n,k:nat | k <= n}
  (tbl: symtbl_t, n: int n, k: int k): void = let
(*
  val () = begin
    print "write_frequencies: [dna_count] starts."; print_newline ()
  end
*)
  val tbl = dna_count (tbl, n, k)
(*
  val () = begin
    print "write_frequencies: [dna_count] is done."; print_newline ()
  end
*)
  var total: int = (0: int)
...
fun write_count {n,k:nat}
  (tbl: symtbl_t, n: int n, seq: string k): void = let
  val k = length seq
  val () = assert (k <= n)
(*
  val () = begin
    print "write_count: [dna_count] starts."; print_newline ()
  end
*)
  val tbl = dna_count (tbl, n, k)
(*
  val () = begin
    print "write_count: [dna_count] is done."; print_newline ()
  end
*)
...
implement getrest () = let
  fun loop {n:nat} (cs: charlst n, n: int n): string_int =
    let val i = getchar () in
      ...
      else (string_make_charlst_int (cs, n), n)
    end
in
  loop (charlst_nil (), 0)
end
...
fun dna_three_get (): string_int =
  let val s = getline () in
    if s <> "" then
