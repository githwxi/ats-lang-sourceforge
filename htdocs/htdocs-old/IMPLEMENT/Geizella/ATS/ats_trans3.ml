...
*)
    ...
    | DE2where (d2e, _) -> type_of_dexp2 d2e
...
*)
  let loc0 = d2e0.dexp2_loc in match d2e0.dexp2_node with
    | DE2ann_type (d2e, s2e) ->
	let d3e = dexp2_tr_dn d2e s2e in
	let () = d3e.dexp3_type <- s2e in d3e
...
and dexp2_arr_tr_up loc0 (s2e_elt: sexp2) (d2es_elt: dexp2 list): dexp3 =
  let n = List.length d2es_elt in
  let d3es_elt = List.map (function d2e -> dexp2_tr_dn d2e s2e_elt) d2es_elt in
  let s2e_arr = sexp2_arraysize_viewt0ype_int_viewt0ype s2e_elt n in
...
and dexp2_item_tr_up loc0 (d2i: ditem2): dexp3 =
  match d2i with
    ...
    | _ -> begin
	P.eprintf
	"%a: dexp2_item_tr_up: the item a constant or variable: <%a>.\n"
...
*)
  ...
  let s2e0 = sexp2_whnf s2e0 in match d2e0.dexp2_node with
    | DE2arr (s2e_elt, d2es_elt) ->
	dexp2_arr_tr_dn loc0 s2e_elt d2es_elt s2e0

    | DE2case (sgn, osty, d2es, c2ls) ->
