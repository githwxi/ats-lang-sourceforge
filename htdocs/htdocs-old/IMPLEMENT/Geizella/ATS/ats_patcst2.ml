...
let rec fprint_patcst2 (out: out_channel) (p2tc: patcst2): unit =
  match p2tc with
  | PTC2any -> P.fprintf out "PTC2any"
  | PTC2bool b -> P.fprintf out "PTC2bool(%b)" b
  | PTC2char c -> P.fprintf out "PTC2char(%c)" c
  | PTC2con (d2c, p2tcs) ->
      P.fprintf out "PTC2con(%a; %a)" fprint_dcon2 d2c fprint_patcst2_list p2tcs
  | PTC2empty -> P.fprintf out "PTC2empty()"
  | PTC2int x -> P.fprintf out "PTC2int(%s)" (string_of_intinf x)
  | PTC2intc xs -> P.fprintf out "PTC2intc(%a)" fprint_intinf_list xs
  | PTC2rec lp2tcs -> P.fprintf out "PTC2rec(%a)" fprint_labpatcst2_list lp2tcs
  | PTC2string s -> P.fprintf out "PTC2string(%s)" s
  | PTC2vbox -> P.fprintf out "PTC2vbox"

and fprint_patcst2_list (out: out_channel) (p2tcs: patcst2 list): unit =
  fprint_list_with_sep fprint_patcst2 out p2tcs ","

and fprint_patcst2_list_list (out: out_channel) (p2tcss: patcst2 list list): unit =
  fprint_list_with_sep fprint_patcst2_list out p2tcss ","

and fprint_labpatcst2 (out: out_channel) ((l, p2tc): labpatcst2): unit =
  P.fprintf out "%a=%a" Lab.fprint l fprint_patcst2 p2tc

and fprint_labpatcst2_list (out: out_channel) (lp2tcs: labpatcst2 list): unit =
...
let rec patcst2_of_pat2 (p2t: pat2): patcst2 =
  match p2t.pat2_node with
    ...
    | PT2vbox _ -> PTC2vbox
    | _ -> begin
	P.fprintf stderr "patcst2_of_pat2: %a\n" fprint_pat2 p2t;
	Err.abort ();
      end
