...
let rec fprint_dexp2 (out: out_channel) (d2e0: dexp2): unit =
  match d2e0.dexp2_node with
    ...
    | DE2where _ -> P.fprintf out "DE2where(...)" 
...
let dcst2_make loc
  ...
  (decarg: decarg2) (arity: int list) (s2e: sexp2) (ext: string option): dcst2 =
...
let dexp2_template loc (d2e: dexp2) (s2ess: sexp2 list list): dexp2 = {
  dexp2_loc= loc;
...
let dexp2_where loc d2e (d2cs: dec2 list): dexp2 = {
  dexp2_loc= loc;
  dexp2_node= DE2where (d2e, d2cs);
  dexp2_type= None;
}
...
let cla2 loc p2ts
  (gua: dexp2 option) (isseq: bool) (isneg: bool) d2e
  : cla2 = {
    cla2_loc= loc;
    cla2_pat= p2ts;
    cla2_gua= gua;
...
let dec2_extype loc (name: string) (def: sexp2): dec2 = {
  dec2_loc= loc;
  dec2_node= DC2extype (name, def)
}

let dec2_extval loc (name: string) (def: dexp2): dec2 = {
  dec2_loc= loc;
  dec2_node= DC2extval (name, def)
}

(* ****** ****** *)

let dec2_data loc (dtk: Syn.datakind) (s2cs: scst2 list): dec2 = {
  dec2_loc= loc;
  dec2_node= DC2data (dtk, s2cs);
}

let dec2_dyncsts loc (dck: Syn.dcstkind) (ds: dcst2 list): dec2 = {
  dec2_loc= loc;
  dec2_node= DC2dyncsts (dck, ds);
}
...
let dec2_impl loc (d2c: dcst2) (decarg: decarg2) (def: dexp2): dec2_impl = {
  dec2_impl_loc= loc;
  dec2_impl_cst= d2c;
...
let dec2_funs loc
  (is_temp: bool) (fk: Syn.funkind) (ds: dec2_fun list): dec2 = {
    dec2_loc= loc;
    dec2_node= DC2funs (is_temp, fk, ds);
  }

(* ****** ****** *)

let moditemdec2_funs loc (fk: Syn.funkind) (ds: dec2_fun list)
  : moditemdec2 = {
    moditemdec2_loc= loc;
    moditemdec2_node= MID2funs (fk, ds);
  }
...
let moditemdec2_vals loc (vk: Syn.valkind) (ds: dec2_val list)
  : moditemdec2 = {
    moditemdec2_loc= loc;
    moditemdec2_node= MID2vals (vk, ds);
  }
