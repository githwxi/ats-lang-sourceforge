...
let metric_add loc (metric: sexp2 list) (metric_bound: sexp2 list): unit =
...
let effenv_check_effset loc0 (effs: Eff.effset): unit =
...
let effenv_add_eff (eff: Eff.effect): unit =
  the_effenv_item_list := EFFENVeff eff :: !the_effenv_item_list

let effenv_add_lam (sf2e: seff2): unit =
...
let scst2_closure loc (stamps: SS.stamps) (s2c: scst2): svar2 * sexp2 list =
...
let sexp2_inst_and_add loc0 (s2vs: svar2 list) (s2ps: sexp2 list): subst =
  let sub =
