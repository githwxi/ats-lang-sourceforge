...
let error_with_loc (loc: loc) (s: string): 'a = begin
  prerr_string "ats_constraint: "; Err.error_with_loc loc s;
end
...
let rec fprint_saexp3 (out: out_channel) (s3ae: saexp3): unit =
  match s3ae with
    ...
    | SAE3padd (s3ae, s3ie) ->
	P.fprintf out "SAE3padd(%a, %a)" fprint_saexp3 s3ae fprint_siexp3 s3ie
    | SAE3null -> P.fprintf out "null"
...
and fprint_sbexp3 (out: out_channel) (s3be: sbexp3): unit =
  match s3be with
    ...
    | SBE3bool b -> if b then P.fprintf out "true" else P.fprintf out "false"

    | SBE3bmul (s3be1, s3be2) ->
	P.fprintf out "SBE3bmul(%a, %a)" fprint_sbexp3 s3be1 fprint_sbexp3 s3be2
    | SBE3badd (s3be1, s3be2) ->
	P.fprintf out "SBE3badd(%a, %a)" fprint_sbexp3 s3be1 fprint_sbexp3 s3be2
    | SBE3bneg s3be -> P.fprintf out "SBE3bneg(%a)" fprint_sbexp3 s3be

    | SBE3ieq (s3ie1, s3ie2) ->
	P.fprintf out "SBE3ieq(%a, %a)" fprint_siexp3 s3ie1 fprint_siexp3 s3ie2
    | SBE3igte (s3ie1, s3ie2) ->
	P.fprintf out "SBE3igte(%a, %a)" fprint_siexp3 s3ie1 fprint_siexp3 s3ie2
    | SBE3ilt (s3ie1, s3ie2) ->
	P.fprintf out "SBE3ilt(%a, %a)" fprint_siexp3 s3ie1 fprint_siexp3 s3ie2
    | SBE3ineq (s3ie1, s3ie2) ->
	P.fprintf out "SBE3igte(%a, %a)" fprint_siexp3 s3ie1 fprint_siexp3 s3ie2

    | SBE3peq (s3ae1, s3ae2) ->
	P.fprintf out "SBE3peq(%a, %a)" fprint_saexp3 s3ae1 fprint_saexp3 s3ae2
    | SBE3pgte (s3ae1, s3ae2) ->
	P.fprintf out "SBE3pgte(%a, %a)" fprint_saexp3 s3ae1 fprint_saexp3 s3ae2
    | SBE3plt (s3ae1, s3ae2) ->
	P.fprintf out "SBE3plt(%a, %a)" fprint_saexp3 s3ae1 fprint_saexp3 s3ae2
    | SBE3pneq (s3ae1, s3ae2) ->
	P.fprintf out "SBE3pneq(%a, %a)" fprint_saexp3 s3ae1 fprint_saexp3 s3ae2
...
and fprint_siexp3 (out: out_channel) (s3ie: siexp3): unit =
  match s3ie with
	...
	P.fprintf out "SIE3iadd(%a, %a)" fprint_siexp3 s3ie1 fprint_siexp3 s3ie2
    | SIE3imul (s3ie1, s3ie2) ->
	P.fprintf out "SIE3imul(%a, %a)" fprint_siexp3 s3ie1 fprint_siexp3 s3ie2
    | SIE3pdiff (s3ae1, s3ae2) ->
	P.fprintf out "SIE3pdiff(%a, %a)" fprint_saexp3 s3ae1 fprint_saexp3 s3ae2

let fprint_sbexp3_list (out: out_channel) (s3bes: sbexp3 list): unit =
...
let fundefs_init (): unit =
   (the_fundef_list := []; the_fundef_list_list := [])

let fundefs_get (): fundef list = !the_fundef_list

let fundefs_pop (): unit = match !the_fundef_list_list with
  | fds :: fdss -> begin
      the_fundef_list := fds;
      the_fundef_list_list := fdss;
    end
...
(*
  let () = P.printf "sbexp3_of_sexp2: s2e = %a\n" fprint_sexp2 s2e in
*)
...
(*
  let () =
    P.fprintf stdout "sbexp3_of_bind: s2v = %a\n" fprint_svar2 s2v in
  let () =
    P.fprintf stdout "sbexp3_of_bind: s2e = %a\n" fprint_sexp2 s2e in
*)
...
(*
  let () = P.printf "siexp3_of_sexp2: s2e = %a\n" fprint_sexp2 s2e in
*)
  let s2e = sexp2_whnf s2e in match s2e.sexp2_node with
    | SE2app (s2e1, s2es2) -> begin
	match s2e1.sexp2_node with
...
(*
    let () =
      P.fprintf stdout "siexp3: VTsizeof: s2es = %a\n" fprint_sexp2_list s2es in
*)
...
(*
    let () =
      P.fprintf stdout "siexp3: VTsizeof: s2v = %a\n" fprint_svar2 s2v in
*)
      Some (SIE3var s2v)
...
(*
  let () = P.printf "saexp3_of_sexp2: s2e = %a\n" fprint_sexp2 s2e in
*)
  let s2e = sexp2_whnf s2e in match s2e.sexp2_node with
...
(*
  let () =
    P.fprintf stdout "fundefs_add: s2c = %a\n" fprint_scst2 s2c in
  let () =
    P.fprintf stdout "fundefs_add: s2es = %a\n" fprint_sexp2_list s2es in
  let () =
    P.fprintf stdout "fundefs_add: s2v = %a\n" fprint_svar2 s2v in
*)
  let s2e_rel =
...
(*
  let () =
    P.fprintf stdout "siexp3_intvec: s3ie = %a\n" fprint_siexp3 s3ie in
*)
...
and siexp3_intvec_aux loc0 (c: intinf)
  ...
  let rec aux (c: intinf) (s3ie: siexp3): unit = match s3ie with
	...
	match scst2ind_find cm s2c with
	  ...
	  | None -> begin
	      P.fprintf stderr "%a: siexp3_intvec_aux: SIE3cst: %a\n"
	...
	match svar2ind_find vm s2v with
	  | Some n -> (vec.(n) <- BI.add_big_int vec.(n) c)
	  | None -> begin
	      P.fprintf stderr "%a: siexp3_intvec_aux: SIE3var: %a\n"
    ...
    | SIE3exp s2e -> begin
	P.fprintf stderr "%a: siexp3_intvec_aux: SIE3exp: %a\n"
...
and saexp3_intvec_aux loc0 (c: intinf)
  ...
  let rec aux (s3ae: saexp3): unit = match s3ae with
	...
	match svar2ind_find vm s2v with
	  | Some n -> (vec.(n) <- BI.add_big_int vec.(n) c)
	  | None -> begin
	      P.fprintf stderr "%a: saexp3_intvec_aux: SAE3var: %a\n"
		Loc.fprint loc0 fprint_svar2 s2v;
	      Err.abort ();
	    end 
      end 
	...
	match scst2ind_find cm s2c with
	  | Some n -> (vec.(n) <- BI.add_big_int vec.(n) c)
	  | None -> begin
	      P.fprintf stderr "%a: saexp3_intvec_aux: SAE3cst: %a\n"
		Loc.fprint loc0 fprint_scst2 s2c;
	      Err.abort ();
	    end
      end
    ...
    | SAE3null -> ()
    | SAE3exp s2e -> begin
	P.fprintf stderr "%a: saexp3_intvec_aux: SAE3exp: %a\n"
	  Loc.fprint loc0 fprint_sexp2 s2e;
	Err.abort ();
      end in
    aux s3ae
...
(*
  let () =
    P.fprintf stdout "sbexp3_icstr: s3be = %a\n" fprint_sbexp3 s3be in
*)
  let rec aux (s3be: sbexp3): FM.icstr = match s3be with
	...
	match scst2ind_find cm s2c with
	  | Some n ->
		...
		FM.ICgte vec
          | None -> begin
	      P.fprintf stderr "%a: sbexp3_icstr: aux: SBE3cst: %a\n"
		Loc.fprint loc0 fprint_scst2 s2c;
	      Err.abort ();
	    end
      end
	...
	match svar2ind_find vm s2v with
	  | Some n ->
		...
		FM.ICgte vec
	  | None -> begin
	      P.fprintf stderr "%a: sbexp3_icstr: aux: SBE3var: %a\n"
		Loc.fprint loc0 fprint_svar2 s2v;
	      Err.abort ();
	    end
      end 
    ...
    | SBE3exp s2e -> begin
	P.fprintf stderr "%a: sbexp3_icstr3: aux: SBE3exp: %a\n"
	  Loc.fprint loc0 fprint_sexp2 s2e;
	Err.abort ();
      end in
    aux s3be
...
(*
  let () =
    P.fprintf stdout "solve_fm: s2vs = %a\n" fprint_svar2_list s2vs in
  let () =
    P.fprintf stdout "solve_fm: s3bes = %a\n" fprint_sbexp3_list s3bes in
  let () =
    P.fprintf stdout "solve_fm: s2p = %a\n" fprint_sexp2 s2p in
*)
...
(*
  let () =
    P.fprintf stdout "solve_fm: ic_asmp:\n%a\n" FM.fprint_icstr_list ics_asmp in
  let () =
    P.fprintf stdout "solve_fm: ic_conc = %a\n" FM.fprint_icstr ic_conc in
*)
    begin
...
let pattern_match_is_nonexhaustive_errmsg loc p2tcs =
  begin
    P.fprintf stderr "%a: nonexhaustive pattern match: %a\n"
...
(*
  let () = P.fprintf stdout "cstr3_solve: s2vs = %a\n" fprint_svar2_list s2vs in
  let () = P.fprintf stdout "cstr3_solve: s3bes = %a\n" fprint_sbexp3_list s3bes in
  let () = P.fprintf stdout "cstr3_solve: c3t = %a\n" fprint_cstr3 c3t in
*)
  ...
  let () =
	...
	| CK_fatal_error -> begin
	    P.fprintf stderr "%a: cstr3_solve: unsolved constraint: %a\n"
...
(*
  let () =
    P.fprintf stdout "cstr3_solve_prop: s2vs = %a\n" fprint_svar2_list s2vs in
  let () =
    P.fprintf stdout "cstr3_solve_prop: s3bes = %a\n" fprint_sbexp3_list s3bes in
  let () =
    P.fprintf stdout "cstr3_solve_prop: s2p = %a\n" fprint_sexp2 s2p in
*)
...
(*
  let () =
    P.fprintf stdout "cstr3_solve_itms: s2vs = %a\n" fprint_svar2_list s2vs in
  let () =
    P.fprintf stdout "cstr3_solve_itms: s3bes = %a\n" fprint_sbexp3_list s3bes in
  let () =
    P.fprintf stdout "cstr3_solve_itms: s3is = %a\n" fprint_sitem3_list s3is in
*)
  match s3is with
    | [] -> ()
...
(*
		  let () =
		    P.fprintf stderr "unused hypothesis: %a\n" fprint_hypo3 h3p in
*)
		    s3bes
	      | Some s3be -> s3be :: s3bes in
...
(*
	    begin
	      P.fprintf stderr "%a: cstr3_solve_itms: s2V = <%a>\n"
		Loc.fprint loc0 fprint_sVar2 s2V;
	      Err.abort ()
	    end
*)
