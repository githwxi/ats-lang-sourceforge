...
fn total_number_of_filled_lines_inc ():<fun> void =
...
fun shape_make {m,n:pos} (M: shape (m, n))
  ...
  val (pf_gc, pf | p) = ptr_alloc_tsz {shapeObj} (sizeof<shapeObj>)
in
  p->rot := 0;
...
in
  ...
  print_frame ();
  curs_invisible () ;
  while (fgetc (file_mode_lte_r_r | !ptr_stdin) <> int_of '\n') () ;
