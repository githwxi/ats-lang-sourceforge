...
// linear type for regular expressions
...
fun regex1_alt (r1: regex1, r2: regex1): regex1 =
  let
    val null: bool = if r1.null then true else r2.null
    val fstpos = r1.fstpos + r2.fstpos
    val lstpos = r1.lstpos + r2.lstpos
  in
    '{ node= REG1alt (r1, r2), null= null, fstpos= fstpos, lstpos= lstpos }
  end

//

fun regex1_chars (x: &T, cs: charset_t): regex1 = 
  let
    val x_len = x.len
    val () = x.len := x_len + 1
    val x_lst = x.lst
...
fun regex1_end (x: &T, irule: int): regex1 =
  let
    val x_len = x.len
    val () = x.len := x_len + 1
    val x_lst = x.lst
...
fun regex1_seq (r1: regex1, r2: regex1): regex1 =
  let
  ...
  in
    '{ node= REG1seq (r1, r2), null= null, fstpos= fstpos, lstpos= lstpos }
  end

//

fun regex1_opt (r: regex1): regex1 =
  let
    val fstpos = r.fstpos and lstpos = r.lstpos
  in
   '{ node= REG1opt r, null= true, fstpos= fstpos, lstpos= lstpos }
  end

fun regex1_plus (r: regex1): regex1 =
  let
    val null = r.null and fstpos = r.fstpos and lstpos = r.lstpos
  in
   '{ node= REG1plus r, null= null, fstpos= fstpos, lstpos= lstpos }
  end

fun regex1_star (r: regex1): regex1 =
  let
    val fstpos = r.fstpos and lstpos = r.lstpos
  in
   '{ node= REG1star r, null= true, fstpos= fstpos, lstpos= lstpos }
  end

//

fun redef_find (env: redef, id0: string): Option regex =
  let
    fun loop (env: redef, id0: string): Option regex =
      case+ env of
        | redef_cons (id, r, env) =>
...
fun regex_mark_str {i,l:nat | i <= l} .<l-i>.
  (env: redef, x0: &T, i: int i, l: int l, s: string l, r1e: regex1): regex1 =
  if i < l then
    let
...
fun regex_mark (env: redef, x0: &T, r0e: regex): regex1 = begin
  case+ r0e of
  | REGalt (r0e1, r0e2) =>
    let
...
and regex_mark_rep
  (env: redef, x0: &T, i: int, r0e: regex, r1e: regex1): regex1 =
  if i > 0 then
    let
...
fun followpos {n:nat} (n0: int n, r: regex1)
  ...
  and aux_node{l:addr}
    ...
    | ~REG1plus r0 => let
        ...
        fn f (pf: !array_v (intset_t, n, l) | i: int):<cloptr1> void =
          let val i = int1_of_int i in
(*
            prerrf ("followpos: i = %i and n = %i\n", @(i,n));
*)
            assert (0 <= i); assert (i < n0); A[i] := A[i] + r0_fstpos
          end
      in
    ...
    | ~REG1seq (r1, r2) => let
        ...
        fn f (pf: !array_v (intset_t, n, l) | i: int):<cloptr1> void =
          let val i = int1_of i in
(*
            prerrf ("followpos: i = %i and n = %i\n", @(i,n));
*)
            assert (0 <= i); assert (i < n0); A[i] := A[i] + r2_fstpos
          end
       in
