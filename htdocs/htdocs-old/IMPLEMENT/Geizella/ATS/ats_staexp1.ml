...
and fprint_e0xp1_list (out: out_channel) (es: e0xp1 list): unit =
...
let fprint_srt1_opt (out: out_channel) (os1t: srt1 option): unit =
  match os1t with
    | None -> ()
    | Some s1t -> fprint_srt1 out s1t
...
let rec fprint_sexp1 (out: out_channel) (s1e0: sexp1): unit =
  match s1e0.sexp1_node with
    ...
    | SE1array (s1e_elt, s1ess_dim) -> P.fprintf out "SE1array(%a; %a)"
...
and fprint_sexp1_list (out: out_channel) (s1es: sexp1 list): unit =
  fprint_list_with_sep fprint_sexp1 out s1es ","

and fprint_sexp1_list_list (out: out_channel) (s1ess: sexp1 list list): unit =
  fprint_list_with_sep fprint_sexp1_list out s1ess ";"

and fprint_sexp1_opt (out: out_channel) (os1e: sexp1 option): unit =
  match os1e with
    | None -> ()
    | Some s1e -> fprint_sexp1 out s1e
...
let sexp1_array loc
  (s1e_elt: sexp1) (s1ess_dim: sexp1 list list): sexp1 = {
    sexp1_loc= loc; sexp1_node= SE1array (s1e_elt, s1ess_dim);
  }
