...
let fprint_intvec (out: out_channel) (v: intvec): unit =
  ...
  let rec aux (i: int): unit =
    if i < n then (P.fprintf out "; %s" (string_of_intinf v.(i)); aux (i+1)) in
    if n > 0 then (P.fprintf out "%s" (string_of_intinf v.(0)); aux 1)

let fprint_intvec_list (out: out_channel) (vs: intvec list): unit =
  fprint_list_with_sep fprint_intvec out vs "\n"

let rec fprint_icstr (out: out_channel) (ic: icstr): unit = match ic with
  | ICeq v -> P.fprintf out "ICeq(%a)" fprint_intvec v
  | ICgte v -> P.fprintf out "ICgte(%a)" fprint_intvec v
  | IClt v -> P.fprintf out "IClt(%a)" fprint_intvec v
  | ICneq v -> P.fprintf out "ICneq(%a)" fprint_intvec v
  | ICconj ics -> P.fprintf out "ICconj(%a)" fprint_icstr_list ics
  | ICdisj ics -> P.fprintf out "ICdisj(%a)" fprint_icstr_list ics
...
(*
  let () =
    P.fprintf stdout "solve_veclist: vs0 =\n%a\n" fprint_intvec_list vs0 in
*)
...
(*
  let () =
    P.fprintf stdout "vec_veceq_elim: v = %a\n" fprint_intvec v in
  let () =
    P.fprintf stdout "vec_veceq_elim: ve = %a\n" fprint_intvec ve in
  let () =
    P.fprintf stdout "vec_veceq_elim: i = %i\n" i in
*)
...
let veclist_veceq_elim (vs: intvec list) (ve: intvec) (i: int): intvec list =
  let rec aux res = function
    | [] -> res
    | v :: vs -> aux (vec_veceq_elim v ve i :: res) vs in
    aux [] vs
