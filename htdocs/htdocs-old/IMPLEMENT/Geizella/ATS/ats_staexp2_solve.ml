...
let error_with_loc (loc: loc) (s: string): 'a = begin
  prerr_string "ats_staexp2_solve: "; Err.error_with_loc loc s;
end
...
let clokind_equal_solve loc0
  ...
  if knd1 <> knd2 then begin
    P.eprintf "%a: function/closure mismatch.\n" Loc.fprint loc0;
    Err.abort ();
    end 
...
let funlinear_equal_solve loc0 (lin1: int) (lin2: int): unit = begin
  if lin1 = lin2 then () else begin
    P.eprintf "%a: function linearity mismatch.\n" Loc.fprint loc0;
    Err.abort ();
  end 
...
*)
  if npf1 < npf2 then begin
    P.eprintf "%a: too few proof argumets.\n" Loc.fprint loc0;
    Err.abort ();
  end else if npf1 > npf2 then begin
    P.eprintf "%a: too many proof argumets.\n" Loc.fprint loc0;
    Err.abort ();
  end else ()
...
*)
  if ptr1 <> ptr2 then begin
    P.eprintf "%a: refarg style mismatch.\n" Loc.fprint loc0;
    Err.abort ();
  end else ()
...
(*
  let () = P.fprintf stdout "sexp2_equal_solve:\n" in
  let () = P.fprintf stdout "  s2e1 = %a\n" fprint_sexp2 s2e1 in
  let () = P.fprintf stdout "  s2e2 = %a\n" fprint_sexp2 s2e2 in
*)
    match s2e10.sexp2_node, s2e20.sexp2_node with
      ...
      | SE2array (s2e1_elt, s2ess1_dim), SE2array (s2e2_elt, s2ess2_dim) ->
	  begin
	  ...
	  if scst2_eq s2c1 s2c2 then () else begin
	    P.eprintf "%a: sexp2_equal_solve: %a <> %a\n"
...
(*
  let () =
    P.fprintf stdout "sexp2_list_equal_solve: s2es1 = %a\n"
      fprint_sexp2_list s2es1 in
  let () =
    P.fprintf stdout "sexp2_list_equal_solve: s2es2 = %a\n"
...
(*
  let () =
    P.fprintf stdout "sexp2_list_list_equal_solve: s2ess1 = %a\n"
      fprint_sexp2_list_list s2ess1 in
  let () =
    P.fprintf stdout "sexp2_list_list_equal_solve: s2ess2 = %a\n"
...
(*
  let () =
    P.fprintf stdout "labsexp2_list_equal_solve: ls2es1 = %a\n"
      fprint_labsexp2_list ls2es1 in
  let () =
    P.fprintf stdout "labsexp2_list_equal_solve: ls2es2 = %a\n"
...
and sexp2_eff_leq_solve loc0 (sf2e1: seff2) (sf2e2: seff2): unit =
(*
  let () =
      P.fprintf stdout "%a: sexp2_eff_leq_solve: %a = %a?\n"
...
*)
  let ans = match sf2e2 with
    ...
    if not (ans) then begin
      P.eprintf "%a: sexp2_eff_leq_solve: %a <!= %a\n"
...
and sexp2_szleq_solve loc0 (s2e1: sexp2) (s2e2: sexp2): unit =
  let sz2e1 = sexp2_sizeof s2e1 and sz2e2 = sexp2_sizeof s2e2 in
    if not (szexp2_leq sz2e1 sz2e2) then
      begin
	P.eprintf "%a: sexp2_szleq_solve: %a != %a\n"
...
and sexp2_szeqeq_solve loc0 (s2e1: sexp2) (s2e2: sexp2): unit =
  let sz2e1 = sexp2_sizeof s2e1 and sz2e2 = sexp2_sizeof s2e2 in
    if not (szexp2_syneq sz2e1 sz2e2) then
      begin
	P.eprintf "%a: sexp2_szeqeq_solve: %a != %a\n"
	  Loc.fprint loc0 fprint_szexp2 sz2e1 fprint_szexp2 sz2e2;
	Err.abort ();
      end
...
and sexp2_szeq_solve_at loc0 (s2e1: sexp2) (s2e2: sexp2): unit =
  let s2e1 = sexp2_whnf s2e1 and s2e2 = sexp2_whnf s2e2 in
    ...
    with
      ...
      | _, _ -> begin
	  P.eprintf "%a: sexp2_szeq_solve_at: %a != %a\n"
...
and sexp2_void_solve loc0 (s2e: sexp2): unit =
  match s2e.sexp2_node with
    ...
    | _ -> begin
	P.eprintf "%a: sexp2_void_solve: not void: %a\n"
...
and sexp2_tyleq_solve loc0 (s2e10: sexp2) (s2e20: sexp2): unit =
  let s2e10 = sexp2_whnf s2e10 and s2e20 = sexp2_whnf s2e20 in
(*
  let () = P.fprintf stdout "sexp2_tyleq_solve:\n" in
...
*)
    match s2e10.sexp2_node, s2e20.sexp2_node with
	  ...
	  if sexp2_is_nonlin s2e10 then begin
	    ...
	    P.eprintf "%a: sexp2_tyleq_solve: %a <!= %a\n"
	      Loc.fprint loc0 fprint_sexp2 s2e10 fprint_sexp2 s2e20;
	    Err.abort ()
	  end
	end
...
and sexp2_tyleq_solve_Var_l loc0 s2V (ub: sexp2): unit =
  if not (sexp2_occurs s2V ub) then
    ...
    P.eprintf "%a: sexp2_tyleq_solve_Var_l: <%a> occurs in <%a>.\n"
...
and sexp2_tyleq_solve_Var_r loc0
  ...
  if not (sexp2_occurs s2V lb) then
	...
	P.eprintf "%a: sexp2_tyleq_solve_Var_r: %a: %a <!= %a\n"
    ...
    P.eprintf "%a: sexp2_tyleq_solve_Var_r: <%a> occurs in <%a>.\n"
...
and sexp2_tyleq_solve_VarApp loc0
  ...
  let s2t_res = bound.sexp2_sort in match bound.sexp2_node with
	...
	| _ -> begin
	    P.eprintf "%a: sexp2_tyleq_solve_VarApp: SE2app: <%a> = <%a>.\n"
    ...
    | _ -> begin
	P.eprintf "%a: sexp2_tyleq_solve_VarApp: <%a> = <%a>.\n"
...
(*
  let () = 
    P.fprintf stdout "sexp2_equal_solve_Var: <%a> <- <%a>.\n"
      fprint_sVar2 s2V1 fprint_sexp2 s2e2 in
  let () = 
    P.fprintf stdout "sexp2_equal_solve_Var: %a.sVar2_svs = (%a)\n"
      fprint_sVar2 s2V1 fprint_stamps s2V1.sVar2_svs in
  let () = 
    P.fprintf stdout "sexp2_equal_solve_Var: fvs(%a) = (%a)\n"
...
*)
  if not (sexp2_occurs s2V1 s2e2) then
    if Stamps.subset (sexp2_fvs_0 s2e2) s2V1.sVar2_svs then
      ...
      let () =
	if not (srt2_leq s2t2 s2t1) then begin
	  P.eprintf "%a: sexp2_equal_solve_Var: %a <- %a: %a <!= %a\n"
      ...
      P.fprintf stdout "%a: sexp2_equal_solve_Var: <%a> <!- <%a>.\n" 
	Loc.fprint loc0 fprint_sVar2 s2V1 fprint_sexp2 s2e2;
      SEnv3.prop_eqeq_add loc0 s2e1 s2e2
    end
  else begin
    P.eprintf "%a: sexp2_equal_solve_Var: <%a> occurs in <%a>.\n"
      Loc.fprint loc0 fprint_sVar2 s2V1 fprint_sexp2 s2e2;
    Err.abort ();
  end 
...
let rec sexp2_hyeq_solve loc0 s2e1 s2e2: unit =
  let s2e1 = sexp2_whnf s2e1 and s2e2 = sexp2_whnf s2e2 in
(*
  let () = P.fprintf stdout "sexp2_hyeq_solve:\n" in
  let () = P.fprintf stdout "  s2e1 = %a\n" fprint_sexp2 s2e1 in
  let () = P.fprintf stdout "  s2e2 = %a\n" fprint_sexp2 s2e2 in
*)
    match s2e1.sexp2_node, s2e2.sexp2_node with
...
(*
  let () = P.fprintf stdout "sexp2_hyeq_solve_con:\n" in
  let () = P.fprintf stdout "  s2e1 = %a\n" fprint_sexp2 s2e1 in
  let () = P.fprintf stdout "  s2e2 = %a\n" fprint_sexp2 s2e2 in
*)
  let rec aux_solve s2e1 s2e2 =
