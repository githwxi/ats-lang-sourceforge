...
let error_with_loc loc (msg: string): 'a =
  Err.error_with_loc loc ("ats_staexp2_util: " ^ msg)
...
and sexp2_list_subst (sub: subst) (s2es: sexp2 list): sexp2 list =
  List.map (sexp2_subst sub) s2es

and sexp2_list_list_subst (sub: subst) (s2ess: sexp2 list list)
  : sexp2 list list =
...
let sexp2_alpha_list (s2v: svar2) (s2v': svar2) (s2es: sexp2 list)
  : sexp2 list = List.map (sexp2_alpha s2v s2v') s2es
...
and sexp2_opt_fvs (fvs: stamps) (bvs: stamps) (os2e: sexp2 option): stamps =
  match os2e with
    | None -> fvs
...
and seff2_fvs (fvs: stamps) (bvs: stamps) (sf2e: seff2): stamps =
  match sf2e with
    | SEFF2all -> fvs
    | SEFF2nil -> fvs
...
and sexp2_topize (s2e0: sexp2): sexp2 =
  let s2e0 = sexp2_whnf s2e0 in match s2e0.sexp2_node with
    | SE2array (s2e_elt, s2ess_dim) -> sexp2_array (sexp2_top s2e_elt) s2ess_dim
...
let rec sexp2_sizeof (s2e0: sexp2): szexp2 =
  let s2t0 = s2e0.sexp2_sort in
    if srt2_is_proof s2t0 then szexp2_zero
      ...
      let s2e0 = sexp2_whnf s2e0 in match s2e0.sexp2_node with
	| SE2array (s2e_elt, s2ess_ind) ->
	    SZE2array (sexp2_sizeof s2e_elt, s2ess_ind)
...
*)
  match sz2e1, sz2e2 with
    | SZE2array (sz2e1_elt, s2ess1_dim), SZE2array (sz2e2_elt, s2ess2_dim) ->
...
*)
    match s2e10.sexp2_node, s2e20.sexp2_node with
      ...
      | SE2array (s2e1_elt, s2ess1_ind), SE2array (s2e2_elt, s2ess2_ind) ->
...
let svar2_is_boxed (s2v: svar2): bool =
...
let sexp2_select_get_ind loc0
  ...
  let s2e_arr = sexp2_whnf s2e_arr in match s2e_arr.sexp2_node with
    | SE2array (s2e_elt, s2ess_dim) -> begin
...
let sexp2_abstract (s2e: sexp2): svar2 list * sexp2 list * sexp2 =
  sexp2_prenex false s2e

let sexp2_list_abstract (s2es: sexp2 list): svar2 list * sexp2 list * sexp2 list =
...
let sexp2_list_open (s2es: sexp2 list): svar2 list * sexp2 list * sexp2 list =
