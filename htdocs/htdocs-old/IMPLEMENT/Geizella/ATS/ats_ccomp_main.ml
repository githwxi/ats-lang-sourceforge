...
let ccomp_main (out: out_channel) (flag: int)
  ...
  let () = FunLabMap.iter aux !the_function_store in
    begin
	...
	P.fprintf out "/* dynamic load function */\n\n";
        let is_dummy = dcst2_fun_dynload_dummy_is_implemented () in
	  cprint_dynload_fun out is_dummy filename tmpset res;
      end;
