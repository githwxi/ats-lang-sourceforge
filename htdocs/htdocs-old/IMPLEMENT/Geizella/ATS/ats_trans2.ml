...
let sexp2_env_qfind (sq: Syn.squal) (id: Syn.sid): sitem2 option =
  match sq with
...
let sexp2_env_add_cst (s2c: scst2): unit =
...
let dexp2_env_find_sym (id: symbol): ditem2 option =
  match SDEnv2.DEXP2env.find id with
    | None -> NS.dexp2_env_find id
    | ans -> ans

let dexp2_env_find (id: Syn.did): ditem2 option =
  dexp2_env_find_sym (Syn.sym_of_did id)

let dexp2_env_file_find_sym (f: Fil.filename) (id: symbol)
  : ditem2 option =
  let name = NS.make_name (Fil.filename_fullname f) in
    match NS.stadynenv_find name with
...
let dexp2_env_file_find (f: Fil.filename) (id: Syn.did): ditem2 option =
  dexp2_env_file_find_sym f (Syn.sym_of_did id)

let dexp2_env_file_sym_find (f: Fil.filename) (s: Syn.sid) (id: Syn.did)
  : ditem2 option =
  match sexp2_env_file_find f s with
    | Some (SI2cst [s2c]) -> dcon2_scst2_find s2c id
    | _ -> None

let dexp2_env_qfind (q: Syn.dqual) (id: Syn.did): ditem2 option =
  match q with
...
let rec srt1_tr (s1t: srt1): srt2 = match s1t.srt1_node with
...
and srt1_app_tr (loc: loc) (s1t: srt1) (s1ts: srt1 list): srt2 =
  ...
  else srt2_app (srt1_tr s1t) (srt1_list_tr s1ts)

(* ****** ****** *)

let effvar_tr loc0 (effv: Eff.effvar): sexp2 =
  match sexp2_env_find_id effv with
    | Some (SI2var s2v) -> sexp2_var (s2v)
    | _ -> begin 
	P.fprintf stderr
...
let effcst_tr loc0 (effc: Eff.effcst): seff2 =
  match effc with
...
let rec sexp1_app_unwind s1e s1ess =
...
*)
    ...
    | _ -> begin
	P.fprintf stderr "%a: sexp1_tr_up: unavailable: %a"
...
(*
	let () =
	  P.fprintf stdout "sexp1_app_id_tr_up: s2cs = %a\n"
	    fprint_scst2_with_sort_list s2cs in
	let () =
	  P.fprintf stdout "sexp1_app_id_tr_up: s2tss = %a\n"
...
and sexp1_app_datcon_tr_up (loc: loc)
  (d2c: dcon2) (s1ess: sexp1 list list): sexp2 =
...
and sexp1_trans_tr_up loc0
  ...
  let (refval, s1e1) = match s1e1.sexp1_node with
    ...
    | _ -> begin
	P.eprintf "%a: sexp1_trans_tr_up: illegal format\n" Loc.fprint loc0;
	Err.abort ()
      end in
  let s2t = if refval = 0 then srt2_view else srt2_viewt0ype in
...
and sexp1_union_tr_up (s1e: sexp1) (ls1es: labsexp1 list): sexp2 =
...
and sexp1_opt_tr_up (os1e: sexp1 option): sexp2 option =
  match os1e with
    | Some s1e -> Some (sexp1_tr_up s1e)
    | None -> None
...
and sexp1_list_tr_dn (loc: loc) (s1es: sexp1 list) (s2ts: srt2 list)
  : sexp2 list = let rec aux s2es = function
    | s1e :: s1es, s2t :: s2ts ->
...
and squas1_tr (squas1: squas1): svar2 list * sexp2 list =
  let rec aux (s2vs0: svar2 list) (s2es0: sexp2 list) (s1qs: squa1 list)
	...
	| SQ1vars (ids, s1te) -> let s2te = srtext1_tr s1te in begin
	    match s2te with
	      | STE2srt s2t ->
		  let s2vs =
		  ...
		  let rec aux1 s2vs0 s2es0 = function
		    ...
		    | [] -> aux s2vs0 s2es0 s1qs in
		    aux1 s2vs0 s2es0 ids
	  end
      end
...
and srtext1_tr (s1te: srtext1): srtext2 = match s1te.srtext1_node with
...
let svar1_tr (id, s1t): svar2 =
  let s2t = srt1_tr s1t in
  let s2v = svar2_new_with_id id s2t in
    (SEnv2.sexp2_env_add_var s2v; s2v)

let svar1_list_tr (s1vs: svar1 list): svar2 list = List.map svar1_tr s1vs

(* ****** ****** *)

let sexparg1_tr (s1a: sexparg1): sexparg2 = match s1a with
...
let srt2_datakind (dtk: Syn.datakind): srt2 =
  match dtk with
...
let rec pat1_tr (p1t: pat1): pat2 =
  let loc = p1t.pat1_loc in match p1t.pat1_node with
    ...
    | _ -> begin
	P.fprintf stderr "%a: pat1_tr: %a: yet to be implemented!"
...
and pat1_vbox_tr loc (np1ts: int_pat1_list): pat2 =
  ...
  let p2t = match p1ts with
    | [p1t] -> pat1_tr p1t
    | _ -> begin
	P.fprintf stderr "%a: exactly one pattern is required.\n"
	  Loc.fprint loc;
	Err.abort ();
      end in
...
let rec dexp1_of_dexp1 (d1e: dexp1): dexp1 =
...
(*
  let () = P.fprintf stdout "dexp1_tr: d1e0 = %a\n" fprint_dexp1 d1e0 in
*)
  let loc = d1e0.dexp1_loc in match d1e0.dexp1_node with
    ...
    | DE1case (sgn, osty1, d1e, c1ls) -> (* sgn: -1, 0, 1 *)
    ...
    | DE1for (oinv1, (init1, test1, post1), body1) ->
    ...
    | DE1if (osty1, d1e1, d1e2, od1e3) ->
	let osty2 = statype1_opt_tr loc osty1 in
	let d2e1 = dexp1_tr d1e1 in
	let d2e2 = dexp1_tr d1e2 in
	let od2e3 = dexp1_opt_tr od1e3 in
	  dexp2_if loc osty2 d2e1 d2e2 od2e3

    | DE1int (ik, i) -> dexp2_int loc ik i

    | DE1lam_dyn (is_lin, p1t, d1e) ->
(*
	let () =
	  P.fprintf stdout "dexp1_tr: DE1lam_dyn: p1t = %a\n" fprint_pat1 p1t in
	let () =
	  P.fprintf stdout "dexp1_tr: DE1lam_dyn: d1e = %a\n" fprint_dexp1 d1e in
*)
    ...
    | DE1lam_sta_syn (squas1, d1e) ->
	let () = SEnv2.SEXP2env.push () in
	let (s2vs, s2es) = squas1_tr squas1 in
	let d2e = dexp1_tr d1e in
	let () = SEnv2.SEXP2env.pop () in
	  dexp2_lam_sta loc s2vs s2es d2e
    | DE1lam_sta_para (squas1, d1e) ->
	let () = SEnv2.SEXP2env.push () in
	let (s2vs, s2es) = squas1_tr squas1 in
	let d2e = dexp1_tr d1e in
	let () = SEnv2.SEXP2env.pop () in
...
*)
    ...
    | DE1trywith (d1e, c1ls) ->
	let d2e = dexp1_tr d1e in
	let c2ls = cla1_list_tr 1 c1ls in
	...
	let d2e = dexp1_tr d1e in
	let () = pop_all () in
	  dexp2_where loc d2e d2cs

    | DE1while (oinv1, test, body) ->
	let () = SEnv2.SEXP2env.push () in
	let oinv2 = loopinv1_opt_tr oinv1 in
	let test = dexp1_tr test in
	let body = dexp1_tr body in
	let () = SEnv2.SEXP2env.pop () in
	  dexp2_while loc oinv2 test body

    | _ -> begin
	P.fprintf stderr "%a: dexp1_tr: d1e0 =  %a\n"
...
(*
  let () =
    P.fprintf stdout "dexp1_arrsub_tr: d2is = %a\n"
...
and dexp1_tr_dn_fun loc
  ...
  match s2e_fun.sexp2_node with
    ...
    | _ -> begin
	P.eprintf "%a: dexp1_tr_dn_fun: s2e_fun = %a\n"
...
(*
  let () =
    P.fprintf stdout "cla1_tr: p2ts = %a\n" fprint_pat2_list p2ts in
*)
  let () =
...
and dec1_datsrt_tr
  ...
  let aux (d1c: datsrtcon1): scst2 =
    ...
    let s2t = match arg with
      | _ :: _ -> srt2_fun arg res
      | [] -> res in
    let s2c =
      scst2_make name
	s2t (* sort *)
...
and dec1_sexpdef_tr_norec (os2t0_res: srt2 option) (d: dec1_sexpdef): scst2 =
  let loc = d.dec1_sexpdef_loc in
...
and dec1_dyncst_tr (dck: Syn.dcstkind)
  ...
  let ext = d.dec1_dyncst_extern in
  let d2c = dcst2_make loc name filename dck decarg arity s2e ext in
(*
  let () =
    P.fprintf stdout "dec1_dyncst_tr: d2c = %a\n" fprint_dcst2 d2c in
  let () =
    P.fprintf stdout "dec1_dyncst_tr: decarg = %a\n" fprint_decarg2 decarg in
*)
...
(*
  let () =
    P.fprintf stdout "dec1_overload_tr: name = %a\n" Syn.fprint_did name in
*)
...
(*
  let () =
    P.fprintf stdout "dec1_overload_tr: d2is = %a\n" fprint_ditem2_list d2is in
*)
...
and dec1_val_tr (p2t: pat2) (d: dec1_val): dec2_val =
  let d2e = dexp1_tr (d.dec1_val_def) in
    match p2t.pat2_node with
      | PT2ann (p2t, s2e) ->
	  dec2_val d.dec1_val_loc p2t d2e (Some s2e)
...
and dec1_impl_tr (d: dec1_impl): dec2_impl =
  ...
  let rec aux2 res args s2vpss s2e = match args with
	...
	| [] -> begin
	    P.fprintf stderr "%a: dec1_impl_tr: aux2: overly applied.\n"
	      Loc.fprint loc;
	    Err.abort ();
	  end
      end in
  let qid = d.dec1_impl_name in match dexp2_env_oqfind qid with
	...
	let () = SDEnv2.stadynenv2_push () in
	let (decarg, s2e_d2c) =
	  aux2 [] d.dec1_impl_decarg d2c.dcst2_decarg d2c.dcst2_type in
        let def = dexp1_tr_dn d.dec1_impl_def s2e_d2c in
        let () = SDEnv2.stadynenv2_pop () in
	let () = d2c.dcst2_def <- Some def in
	  dec2_impl d.dec1_impl_loc d2c decarg def
    | _ -> dec1_impl_tr_errmsg_qid d.dec1_impl_loc qid

and dec1_impl_list_tr (ds: dec1_impl list): dec2_impl list =
  List.map dec1_impl_tr ds
...
(*
  let () =
    P.fprintf stdout "dec1_staload_tr: fullname = %s\n" fullname in
*)
...
and dec1_list_tr_aux1 (ds: dec1 list) (res: dec2 list): dec2 list =
  match ds with
    | d :: ds -> dec1_list_tr_aux2 d ds res
    | [] -> List.rev res

and dec1_list_tr_aux2 (d: dec1) (ds: dec1 list) (res: dec2 list)
  : dec2 list = match d.dec1_node with
  ...
  | DC1impls xs ->
      let xs = dec1_impl_list_tr xs in
