...
*)
  match e.e0xp1_node with
    ...
    | E0XP1list _ -> e0xp1_eval_errmsg_list e.e0xp1_loc
...
let rec sexp_tr (se: sexp): sexp1 =
  ...
  and to_items (se: sexp): sexp1_item list =
    ...
    match to_item se with
      | Fix.Atm s1e ->
(*
	  let () = P.fprintf stdout "sexp_tr: %a\n" fprint_sexp1 s1e in
*)
	    s1e
...
and sexp_list_list_opt_tr (osess: (sexp list list) option)
  : (sexp1 list list) option = match osess with
    | None -> None
    | Some sess -> Some (sexp_list_list_tr sess)
...
let backslash_dexp1_item : dexp1_item =
  Fix.make_backslash_opr
    (function d1e -> d1e.dexp1_loc) dexp1_app_dyn0
...
let dexp1_term_check loc
  ...
  match oeffc with
	...
	then begin
	  P.eprintf "%a: a termination metric is missing.\n"
	    Loc.fprint loc;
	  Err.abort ();
	end
    | None -> ()
  ...
  let rec to_item (de: dexp): dexp1_item =
    let loc = de.dexp_loc in match de.dexp_node with
	  ...
	  let d1e = dexp_tr de in match d1e.dexp1_node with
	    ...
	    | _ -> begin
                P.eprintf "%a: the dynamic expression needs to be a string constant.\n"
      ...
      | DEparse (id, s) ->
(*
	  let () = P.fprintf stdout "dexp_tr: DEparse: id = %a\n" fprint_id id in
	  let () = P.fprintf stdout "dexp_tr: DEparse: s = %s\n" s in
*)
	  let s_new =
	    let parser = match Ats_string_parser.parfun_find id with
	      | Some f -> f
	      | None -> begin
		  P.eprintf "%a: there is no parse function for <%a>\n"
		...
		| Some s -> s
		| None -> begin
		    P.eprintf "%a: illegal string for <%a>\n"
		      Loc.fprint loc fprint_id id;
		    Err.abort ();
		  end in
(*
	  let () = P.fprintf stdout "dexp_tr: DEparse: s_new = %s\n" s_new in
*)
	  let prompt out = P.fprintf out "%a: " Loc.fprint loc in
...
and dexp_lams_dyn_tr
  ...
  (de_body: dexp): dexp1 =
  let rec aux (flag: int) (fargs: farg list) (d1e_body: dexp1): dexp1 =
    match fargs with
      | farg :: fargs ->
...
(*
  let () =
    P.fprintf stdout "dec_dyncst_tr_aux2: s1e_res = %a\n"
...
*)
  ...
  let () = match fc with
    | FUNCLOclo knd ->
        if knd >= 0 then begin
          P.eprintf "%a: a clousure at the toplevel must be a reference"
	    Loc.fprint loc_res;
	  Err.abort ()
        end
    | FUNCLOfun -> () in
...
and dec_dyncst_tr
  ...
  let filename = d.dec_dyncst_filename in
    dec1_dyncst d.dec_dyncst_loc
      d.dec_dyncst_name filename s1e d.dec_dyncst_extern
...
and dec_fun_list_tr (fk: funkind) (ds: dec_fun list): dec1_fun list =
  List.map (dec_fun_tr fk) ds

and dec_impl_tr (d: dec_impl): dec1_impl =
  let decarg = sarg_list_list_tr d.dec_impl_decarg in
  let body =
    dexp_lams_dyn_tr None (*loc*) None (*fc*) false (*lin*)
      d.dec_impl_arg d.dec_impl_res None d.dec_impl_body in
    dec1_impl d.dec_impl_loc d.dec_impl_name decarg body
...
and dec_list_tr_aux2 (d: dec) (ds: dec list) (res: dec1 list)
  ...
  | DCe0xperr e -> begin
	...
	| V0AL1char c -> (P.eprintf "'%c'" c; Err.abort ())
	| V0AL1float f -> (P.eprintf "%f" f; Err.abort ())
	| V0AL1int i -> (fprint_intinf stderr i; Err.abort ())
	| V0AL1str s -> (P.eprintf "%s" s; Err.abort ())
    end
  ...
  | DCe0xpprt e -> begin
      ...
      let () = match v with
        | V0AL1char c -> P.eprintf "'%c'" c
        | V0AL1float f -> P.eprintf "%f" f
	| V0AL1int i -> fprint_intinf stderr i
	| V0AL1str s -> P.eprintf "%s" s in
	dec_list_tr_aux1 ds res
    end
  ...
  | DCimpls xs ->
      ...
      let d1 = dec1_impls d.dec_loc xs in
	dec_list_tr_aux1 ds (d1 :: res)
...
(*
  let () = P.printf "Including %s begins.\n" fname in
*)
  let res = dec_list_tr_aux1 d0cs res in
(*
  let () = P.printf "Translating %s ends.\n" fname in
*)
    res
...
and dec_staload_tr (f: Fil.filename): bool * trans1_res =
  ...
  let fsymbol = Sym.make_with_string fname in
    match staloadedFileMapFind fsymbol with
	  ...
	  let () = Fil.filename_pop () in
	  let () = P.printf "Translating %s begins.\n" fname in
      ...
      | Some res ->
(*
	  let _ = P.printf "staloaddec0trans: already loaded: %s\n" fullname in
*)
	  (true (*is_loaded*), res)
