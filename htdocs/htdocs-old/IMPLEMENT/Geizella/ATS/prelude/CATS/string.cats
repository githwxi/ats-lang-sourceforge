...
static inline
ats_ptr_type
atspre_strbuf_make_uninitialized (const ats_int_type n) {
  char *p ;
  p = ats_malloc_gc(n+1); p[n] = '\000'; return p ;
}

static inline
ats_ptr_type
atspre_strbuf_make_char(const ats_int_type n, const ats_char_type c) {
  char *p ; 
  p = ats_malloc_gc(n+1) ; memset (p, c, n) ; p[n] = '\000' ;
  return p ;
}
...
atspre_strbuf_contains
  ...
  return (strchr((char *)s, (char)c) != (char *)0) ;
}
...
atspre_string_tolower (const ats_ptr_type s) {
  ...
  des0 = ats_malloc_gc(n+1) ; des = des0 ;
