...
let fprint_squal_id (out: out_channel) ((sq, id): squal_id)
  : unit = (fprint_squal out sq; fprint_sid out id)
...
let fprint_funclo (out: out_channel) (fc: funclo): unit =
  match fc with
    | FUNCLOclo (knd) -> P.fprintf out "clo(%i)" knd
    | FUNCLOfun -> P.fprintf out "fun"
...
let fprint_effect (out: out_channel) (eff: effect): unit =
  match eff with
    | EFFall -> P.fprintf out "all"
    | EFFexn -> P.fprintf out "exn"
    | EFFntm -> P.fprintf out "ntm"
    | EFFref -> P.fprintf out "ref"
...
let fprint_efftag
 (out: out_channel) (tag: efftag): unit = begin
  match tag with
  | EFFTAGcst name -> P.fprintf out "!%s" name
  | EFFTAGvar id -> fprint_sid out id
  | EFFTAGprf -> P.fprintf out "proof"
  | EFFTAGlin i -> P.fprintf out "lin%i" i
  | EFFTAGfun i -> P.fprintf out "fun%i" i
  | EFFTAGcloptr i -> P.fprintf out "cloptr%i" i
  | EFFTAGcloref i -> P.fprintf out "cloref%i" i
...
let sexp_array (se_elt: sexp) (sess_dim: sexp list list): sexp = {
  sexp_loc= getloc ();
  sexp_node= SEarray (se_elt, sess_dim);
}
...
let sexp_tyrec (is_flat: bool) (lses: labsexp list): sexp = {
  sexp_loc= getloc ();
...
let svar_of_sarg ((id, ost): sarg): svar =
  match ost with
    | Some st -> (id, st)
    | None -> begin
	P.eprintf "%a: sort annotation for <%a> is required.\n"
...
and dec_dyncst = {
  ...
  dec_dyncst_res: sexp;
  dec_dyncst_extern: string option;
}
...
and dec_impl = {
  ...
  dec_impl_decarg: sarg list list;
...
and dcstkind = DCKval | DCKfun | DCKprval | DCKpraxi
...
let dexp_sel_lab_ind (is_ptr: int) (l: label) (ind: dexp list list ): dexp =
...
let dec_abstypes
  ...
  let f (sid, arg, def): dec_stacon =
    let () = match def with
      | None -> ()
      | Some _ -> begin
	  P.eprintf "%a: abstype cannot have external definition.\n"
	    Loc.fprint loc;
	  Err.abort ()
	end in
...
let dec_absviewtypes
  ...
  let f (sid, arg, def): dec_stacon =
    let () = match def with
      | None -> ()
      | Some _ -> begin
	  P.eprintf "%a: absviewtype cannot have external definition.\n"
	    Loc.fprint loc;
	  Err.abort ()
	end in
      dec_stacon sid arg def in
...
let dcstkind_is_fun (dck: dcstkind): bool =
  match dck with DCKfun -> true | _ -> false

let dcstkind_is_proof (dck: dcstkind): bool =
  match dck with DCKprval -> true | DCKpraxi -> true | _ -> false
...
let dec_impl
  ((name: dqual_opt_id), (decarg: sarg list list))
  (arg: farg list) (res: sexp option) (body: dexp): dec_impl = {
    dec_impl_loc= getloc ();
    dec_impl_name= name;
    dec_impl_decarg= decarg;
    dec_impl_arg= arg;
    dec_impl_res= res;
    dec_impl_body= body;
  }

let dec_impls (ds: dec_impl list): dec = 
  { dec_loc= getloc (); dec_node= DCimpls ds; }
