...
fun print_expr (e: expr): void =
  case+ e of
    | Num r => print r
    | Add (e1, e2) =>
      let val p1 = priority_mac e1 and p2 = priority_mac e2 in
...
fun playGame (cs: cards, res: rat_t): Bool =
  let
    fun aux_main (zs: cards, xys: ratpairs):<cloptr1> Bool =
      case- zs of
        | x :: nil () => 
	  if res = val_of_card_mac x then begin
            (print_card_mac x; print " = "; print res; print_newline (); true)
          end else false
...
and loop4 (i1: int, i2: int, i3: int, i4: int): void =
  if i4 <= BOUND then
    (ignore (play (i1, i2, i3, i4)); loop4 (i1, i2, i3, i4+1))
  else loop3 (i1, i2, i3+1)
