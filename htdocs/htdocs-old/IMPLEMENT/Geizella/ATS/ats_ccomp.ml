...
type tmpvar = {
  ...
  tmpvar_isali: bool; (* alias status *)
...
let valprim_is_const (vp: valprim): bool =
  match vp.valprim_node with
    | VPbool _ -> true
    | VPchar _ -> true
    | VPcst _ -> true
...
let tmpvar_ali_new (hit: hityp): tmpvar =
  let hit = hityp_nf hit in
  let stamp = Cnt.tmpvar_new_stamp () in {
      tmpvar_type= hit;
      tmpvar_root= None;
...
let tmpvar_new_with_root (tmp: tmpvar): tmpvar =
  ...
  let stamp = Cnt.tmpvar_new_stamp () in {
      ...
      tmpvar_isali= tmp_root.tmpvar_isali;
...
let funlab_set_of_function (f0: function_t): FunLabSet.t =
  let level = f0.function_level in
  let rec aux flset = function
    | fl :: fls ->
	let f = function_store_find fl in
	  if f.function_level < level then
