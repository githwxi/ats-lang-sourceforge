...
let rec code_dexp2_ev loc0 (env: macenv_t) (d2e0: dexp2): dexp2 =
  let rec aux d2e0 = match d2e0.dexp2_node with
    ...
    | DE2lam_dyn (is_lin, (npf, p2ts), d2e) ->
	let (env, p2ts) = code_pat_list_ev loc0 env p2ts in
	let d2e = code_dexp2_ev loc0 env d2e in
	...
	let (env, d2cs) = code_dec2_list_ev loc0 env d2cs in
	let d2e = code_dexp2_ev loc0 env d2e in
	  dexp2_where loc0 d2e d2cs 

    | DE2while (inv, test, body) -> 
	let test = code_dexp2_ev loc0 env test in
	let body = code_dexp2_ev loc0 env body in
 ...
 *)
  ...
  and aux_var (d2v0: dvar2): dexp2 = 
    match macenv_find env d2v0 with
      | None -> dexp2_var loc0 d2v0
      | Some v -> dexp2_of_value2 loc0 v

  and aux_list (d2es0: dexp2 list): dexp2 list =
    List.map aux d2es0

  and aux_list_list (d2ess0: dexp2 list list): dexp2 list list =
    List.map aux_list d2ess0

  and aux_lablist (ld2es0: labdexp2 list): labdexp2 list =
    List.map (function (l, d2e) -> (l, aux d2e)) ld2es0

  and aux_opt (od2e0: dexp2 option): dexp2 option =
    match od2e0 with
      | None -> None
      | Some d2e -> Some (aux d2e)
  ...
  and aux_sty (sty2: statype2): statype2 =
    begin
...
and code_dexp_list_ev loc0 (env: macenv_t) (d2es: dexp2 list)
  : dexp2 list = List.map (code_dexp2_ev loc0 env) d2es

and code_dexp_opt_ev loc0 (env: macenv_t) (od2e: dexp2 option)
  : dexp2 option = match od2e with
    | None -> None
    | Some d2e -> Some (code_dexp2_ev loc0 env d2e)

and code_dexparg_ev loc0 (env: macenv_t) (d2a: dexparg2): dexparg2 =
  match d2a with
    | DEXPARG2sta _ -> d2a
    | DEXPARG2dyn (npf, d2es) ->
	let d2es = code_dexp_list_ev loc0 env d2es in
	  DEXPARG2dyn (npf, d2es)

and code_dexparg_list_ev loc0 (env: macenv_t) (d2as: dexparg2 list)
  : dexparg2 list =
  List.map (code_dexparg_ev loc0 env) d2as

and code_cla_ev loc0 (env: macenv_t) (c2l: cla2): cla2 =
  let (env, p2ts) = code_pat_list_ev loc0 env (c2l.cla2_pat) in
...
and code_dec2_impl_list_ev loc0 (env: macenv_t) (ds: dec2_impl list)
  ...
  let aux d =
    let def = code_dexp2_ev loc0 env d.dec2_impl_def in
      dec2_impl loc0 d.dec2_impl_cst d.dec2_impl_decarg def in
    List.map aux ds
...
and dexp_list_ev loc0 (env: macenv_t) (d2es: dexp2 list): value2 list =
...
let macro_decode_abbrev loc0 (d2m: dmac2) (d2as: dexparg2 list): dexp2 =
