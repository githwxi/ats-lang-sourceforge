  ...
  let rec aux i ctx res hips_arg hits_arg =
    match hips_arg, hits_arg with
	  ...
	  match hip_arg.hipat_node with
		...
		let ctx = varctx_add ctx d2v vp_ptr in
		  match ohip with
		    | Some hip ->
...
and ccomp_exp_template_cst loc (res: instr_t list)
  ...
  let basename = match d2c0.dcst2_extern with
    | Some name -> name | None -> d2c0.dcst2_fullname_id in
...
and ccomp_exp_with_tmpvar (ctx: varctx)
  ...
  let hit0 = hie0.hiexp_type in match hie0.hiexp_node with
	...
	let is_tail =
	  if tmp_res.tmpvar_isret then begin
	    match vp_fun.valprim_node with
	      | (VPclo (_, fl, _) | VPfun fl) -> tailjoin_list_find fl
	      | _ -> None
	  end else None in
	...
	let brs = ccomp_hicla_list k_fail_opt ctx tmp_res vps hicls in
	  iswitch res brs
    | HIEchar c -> imove_val_char res tmp_res c
...
(*
  ...
  viewtypedef arraysize_viewtype_int_viewtype (a: viewtype, n:int) =
  [l:addr | l <> null] (@[a][n] @ l | ptr l, int n)
...
and ccomp_hicla_list (k_fail_opt: cont_t option) (ctx: varctx)
  ...
  let l_first = tmplab_new () in match hicls with
    ...
    | [] -> begin
	let br = (l_first, [Iextern "ats_match_failure ()"]) in [br]
      end
...
and ccomp_dec_val (is_exhaustive: bool) (level: int)
  ...
  let k_fail_opt =
    if is_exhaustive then None else Some (CONTmatch_fail) in
    ccomp_match level k_fail_opt ctx res vp d.hidec_val_pat
