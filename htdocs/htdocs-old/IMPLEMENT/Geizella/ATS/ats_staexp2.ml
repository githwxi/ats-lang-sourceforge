...
let error_with_loc loc (msg: string): 'a =
  Err.error_with_loc loc ("ats_staexp2: " ^ msg)
...
and sexp2_node =
  | SE2app of (* static application *)
      sexp2 * sexp2 list
  | SE2array of (* array (element, dimension) *)
...
let fprint_srt2_var (out: out_channel) (s2tv: srt2_var): unit =
  Syn.fprint_srt_id out s2tv.srt2_var_name

let fprint_srt2_var_list (out: out_channel) (s2tvs: srt2_var list): unit =
 fprint_list_with_sep fprint_srt2_var out s2tvs ","

let fprint_srt2_dat (out: out_channel) (s2td: srt2_dat): unit =
...
and fprint_srt2_list (out: out_channel) (s2ts: srt2 list): unit =
  fprint_list_with_sep fprint_srt2 out s2ts ","

and fprint_srt2_list_list (out: out_channel) (s2tss: srt2 list list): unit =
...
let fprint_scst2_with_sort_list (out: out_channel) (s2cs: scst2 list): unit =
...
let fprint_svar2_list (out: out_channel) (s2vs: svar2 list): unit =
...
let fprint_dcon2_list (out: out_channel) (d2cs: dcon2 list): unit =
...
let fprint_sitem2_opt (out: out_channel) (os2i: sitem2 option): unit =
  match os2i with
    | None -> ()
    | Some s2i -> fprint_sitem2 out s2i
...
let rec fprint_sexp2 (out: out_channel) (s2e: sexp2): unit =
  match s2e.sexp2_node with
    ...
    | SE2array (s2e_elt, s2ess_dim) -> P.fprintf out "SE2array(%a; %a)"
...
and  fprint_sexp2_list_list (out: out_channel) (s2ess: sexp2 list list): unit =
  fprint_list_with_sep fprint_sexp2_list out s2ess ";"

and  fprint_sexp2_opt (out: out_channel) (os2e: sexp2 option): unit =
  match os2e with
    | None -> ()
    | Some s2e -> fprint_sexp2 out s2e
...
and fprint_szexp2 (out: out_channel) (sz2e: szexp2): unit =
  match sz2e with
    | SZE2array (sz2e, s2ess_dim) -> P.fprintf out "SZE2array(%a; %a)" 
...
and fprint_labszexp2  (out: out_channel) ((l, sz2e): labszexp2): unit =
  P.fprintf out "%a= %a" Lab.fprint l fprint_szexp2 sz2e

and  fprint_labszexp2_list (out: out_channel) (lsz2es: labszexp2 list): unit =
  fprint_list_with_sep fprint_labszexp2 out lsz2es ","

(* ****** ****** *)

and fprint_labsvar2 (out: out_channel) ((l, s2v): labsvar2): unit =
  P.fprintf out "%a= %a" Lab.fprint l fprint_svar2 s2v

and  fprint_labsvar2_list (out: out_channel) (ls2vs: labsvar2 list): unit =
  fprint_list_with_sep fprint_labsvar2 out ls2vs ","

(* ****** ****** *)

and fprint_labsexp2 (out: out_channel) ((l, s2e2): labsexp2): unit =
  P.fprintf out "%a= %a" Lab.fprint l fprint_sexp2 s2e2

and  fprint_labsexp2_list (out: out_channel) (ls2es: labsexp2 list): unit =
...
and fprint_slab2_list (out: out_channel) (s2ls: slab2 list): unit =
...
and fprint_labskexp2 (out: out_channel) ((l, sk): labskexp2): unit =
  P.fprintf out "%a= %a" Lab.fprint l fprint_skexp2 sk

and fprint_skexp2_list (out: out_channel) (sks: skexp2 list)
  : unit = fprint_list_with_sep fprint_skexp2 out sks ","

and fprint_labskexp2_list (out: out_channel) (lsks: labskexp2 list)
  : unit = fprint_list_with_sep fprint_labskexp2 out lsks ","

let fprint_skexp2_list_list (out: out_channel) (skss: skexp2 list list): unit =
...
let sexp2_array
  ...
  sexp2_node= SE2array (s2e_elt, s2ess_dim);
}
...
let sexp2_tup (s2es: sexp2 list): sexp2 =
  let s2ts =
    List.map (function s2e -> s2e.sexp2_sort) s2es in
    { sexp2_sort= SRT2tup s2ts; sexp2_node= SE2tup s2es; }

(* ****** ****** *)

(* sexp2_list: [s2es] consists of types, props, views and viewtypes *)
